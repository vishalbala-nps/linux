...
static int aw882xx_get_sysint(struct aw882xx *aw882xx, uint16_t *sysint)
{
	int ret = -1;
	unsigned int reg_val = 0;
...
{
	aw_snd_soc_codec_t *codec = snd_kcontrol_chip(kcontrol);
	struct aw882xx *aw882xx =
...
{
	aw882xx->reset_gpio = devm_gpiod_get(aw882xx->dev, "reset", GPIOD_OUT_LOW);
	if (IS_ERR(aw882xx->reset_gpio)) {
		aw_dev_err(aw882xx->dev,
			"%s: no reset gpio provided, will not HW reset device\n",
			__func__);
		aw882xx->reset_gpio = NULL;
	} else {
		aw_dev_info(aw882xx->dev, "%s: reset gpio provided ok\n",
			__func__);
	}
	aw882xx->irq_gpio = devm_gpiod_get(aw882xx->dev, "irq", GPIOD_IN);
	if (IS_ERR(aw882xx->irq_gpio)) {
		aw_dev_err(aw882xx->dev, "%s: no irq gpio provided.\n",
			__func__);
		aw882xx->irq_gpio = NULL;
	} else
...
static void aw882xx_hw_reset(struct aw882xx *aw882xx)
{
    aw_dev_info(aw882xx->dev, "%s: enter\n", __func__);
    if (IS_ERR(aw882xx->aw_pinctrl)) {
        if (aw882xx->reset_gpio) {
            gpiod_set_value_cansleep(aw882xx->reset_gpio, 0);
            msleep(1);
            gpiod_set_value_cansleep(aw882xx->reset_gpio, 1);
            msleep(2);
        } else {
